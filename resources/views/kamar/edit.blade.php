...
<div class="flex items-end justify-between mb-5">
    <div>
        <h1 class="text-[17px] font-medium text-[#121212]">Edit Kamar</h1>
        <p class="text-[12px] text-[#999] mt-0.5">Perbarui data kamar</p>
    </div>
    ...
    <form action="{{ route('kamar.update', $kamar->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="p-5 grid grid-cols-2 gap-x-6 gap-y-5">
            ...
            <div class="col-span-2 md:col-span-1">
                ...
                <select name="tipe_kamar"
                    ...
                    @foreach(['Standard','Deluxe','Suite','Superior','Family'] as $t)
                        <option value="{{ $t }}" {{ old('tipe_kamar', $kamar->tipe_kamar) === $t ? 'selected' : '' }}>
                            {{ $t }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-span-2 md:col-span-1">
                ...
                @if ($kamar->foto)
                    <div class="flex items-center gap-3 mb-2 p-2.5 bg-[#FAFAF9] border border-black/[0.06] rounded-[7px]">
                        <img src="{{ asset('img/kamar/' . $kamar->foto) }}"
                             alt="{{ $kamar->nama_kamar }}"
                             class="w-14 h-10 object-cover rounded-[5px] flex-shrink-0">
                        <div>
                            <p class="text-[11.5px] font-medium text-[#464646]">Foto saat ini</p>
                            <p class="text-[10px] text-[#bbb] mt-0.5">Upload baru untuk mengganti</p>
                        </div>
                    </div>
                @endif
                ...
                <label class="flex items-center gap-2.5 w-full bg-[#FAFAF9] border border-black/[0.08] border-dashed rounded-[7px] px-3 py-[8px] cursor-pointer hover:bg-[#F5F4F2] hover:border-[#FF6B00]/40 transition-colors group">
                    ...
                    <span class="text-[12px] text-[#bbb] group-hover:text-[#888] transition-colors" id="foto-label">
                        {{ $kamar->foto ? 'Ganti foto...' : 'Pilih foto (jpg, png, webp)' }}
                    </span>
            ...
            <div class="col-span-2">
                ...
                <textarea name="deskripsi" rows="3"
                          ...
                          class="w-full text-[12.5px] text-[#121212] placeholder-[#ccc] bg-[#FAFAF9] border border-black/[0.08] rounded-[7px] px-3 py-2 outline-none resize-none focus:border-[#FF6B00] focus:bg-white transition-colors">{{ old('deskripsi', $kamar->deskripsi) }}</textarea>
            </div>
        ...
        <div class="px-5 py-3.5 border-t border-black/[0.06] bg-[#FAFAF9] flex items-center justify-between">
            <p class="text-[11px] text-[#bbb]">
                ...
                Dibuat {{ $kamar->created_at->diffForHumans() }}
            </p>
