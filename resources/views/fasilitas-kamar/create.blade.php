...
<div class="flex items-end justify-between mb-5">
    <div>
        <h1 class="text-[17px] font-medium text-[#121212]">Tambah Fasilitas Kamar</h1>
        <p class="text-[12px] text-[#999] mt-0.5">Tambah fasilitas untuk kamar tertentu</p>
    </div>
    ...
    <form action="{{ route('fasilitas-kamar.store') }}" method="POST">
        ...
        <div class="p-5 flex flex-col gap-5">
                ...
                <select name="kamar_id"
                    ...
                    @foreach ($kamars as $kamar)
                        <option value="{{ $kamar->id }}" {{ old('kamar_id') == $kamar->id ? 'selected' : '' }}>
                            #{{ $kamar->id }} — {{ $kamar->nama_kamar }} ({{ $kamar->tipe_kamar }})
                        </option>
                    @endforeach
                </select>
                @error('kamar_id')
            ...
            <div>
                ...
                <p class="text-[10px] text-[#bbb] mt-1.5">Klik chip di atas untuk isi cepat, atau ketik manual.</p>
            </div>
