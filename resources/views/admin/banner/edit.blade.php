...
<div class="flex items-end justify-between mb-5">
    <div>
        <h1 class="text-[17px] font-medium text-[#121212]">Edit Banner</h1>
        <p class="text-[12px] text-[#999] mt-0.5">Perbarui informasi banner</p>
    </div>
...
<div class="bg-white border border-black/[0.06] rounded-[10px] overflow-hidden">
    ...
    <form action="{{ route('admin.banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="p-5 flex flex-col gap-5">
            ...
            <div>
                <label class="block text-[11.5px] font-medium text-[#464646] mb-1.5">
                    Gambar <span class="text-[11px] font-normal text-[#bbb]">(kosongkan jika tidak diganti)</span>
                </label>

                {{-- Preview gambar saat ini --}}
                @if ($banner->gambar)
                    <div class="mb-2.5 flex items-center gap-3 p-3 bg-[#FAFAF9] border border-black/[0.06] rounded-[7px]">
                        <div class="w-[72px] h-[46px] rounded-[5px] overflow-hidden bg-[#F0EFED] border border-black/[0.06] flex-shrink-0">
                            <img src="/img/banner/{{ $banner->gambar }}"
                                 alt="{{ $banner->judul }}"
                                 class="w-full h-full object-cover">
                        </div>
                        <div>
                            <p class="text-[11.5px] font-medium text-[#464646]">Gambar saat ini</p>
                            <p class="text-[10.5px] text-[#bbb] mt-0.5">{{ $banner->gambar }}</p>
                        </div>
                    </div>
                @endif
