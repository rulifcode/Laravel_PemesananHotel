    ...
    <form action="{{ route('admin.banner.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="p-5 flex flex-col gap-5">
            ...
            <div>
                ...
                <input type="text" name="judul" value="{{ old('judul') }}" placeholder="cth. Promo Akhir Tahun"
                ...
                @error('judul')<p class="text-[11px] text-[#E24B4A] mt-1">{{ $message }}</p>@enderror
            </div>

            {{-- Media --}}
            <div>
                <label class="block text-[11.5px] font-medium text-[#464646] mb-1.5">
                    Media <span class="text-[#FF6B00]">*</span>
                </label>

                <div id="preview-wrap" class="hidden mb-3 rounded-[8px] overflow-hidden border border-black/[0.08] bg-[#F5F4F2] max-h-[200px]">
                    <img id="preview-img" class="hidden w-full max-h-[200px] object-cover" alt="preview">
                    <video id="preview-vid" class="hidden w-full max-h-[200px] object-cover" muted autoplay loop playsinline></video>
                </div>

                <input type="file" name="media" id="media-input" accept="image/*,video/mp4,video/webm,video/ogg"
                       class="w-full text-[12px] text-[#464646] bg-[#FAFAF9] border rounded-[7px] px-3 py-[7px] outline-none transition-colors cursor-pointer
                              ...
                              {{ $errors->has('media') ? 'border-[#E24B4A] bg-[#FEF0F0]' : 'border-black/[0.08] focus:border-[#FF6B00] focus:bg-white' }}">
                @error('media')<p class="text-[11px] text-[#E24B4A] mt-1">{{ $message }}</p>@enderror
                <p class="text-[10px] text-[#bbb] mt-1.5">Format: JPG, PNG, WEBP, GIF, MP4, WEBM, OGG &mdash; Maks. 20 MB &mdash; Rasio ideal 16:5.</p>
            </div>
            ...
            <div>
                ...
                @error('link')<p class="text-[11px] text-[#E24B4A] mt-1">{{ $message }}</p>@enderror
            </div>

            {{-- Urutan --}}
            <div>
                <label class="block text-[11.5px] font-medium text-[#464646] mb-1.5">Urutan</label>
                ...
                @error('urutan')<p class="text-[11px] text-[#E24B4A] mt-1">{{ $message }}</p>@enderror
        ...
        <div class="px-5 py-3.5 border-t border-black/[0.06] bg-[#FAFAF9] flex items-center justify-end gap-2">
            ...
            <button type="submit"
                ...
                <i class="bi bi-check-lg text-sm"></i> Simpan Banner
            </button>
        </div>
    </form>
</div>
